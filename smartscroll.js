(function($) {
  ...
  $.smartscroll = function(overrides) {
    ...
    var sectionWrapperIsVisible = function() {
      ...
      if (
        windowBottom > sectionWrapperTop && windowTop <= sectionWrapperBottom
      ) {
        return true;
      }
      return false;
    ...
    var scrollToPixel = function(pixel, speed) {
      ...
      $('body,html').stop(true, true).animate({
        ...
        if (options.eventEmitter) {
          var visibleSection = getSectionIndexAt($(window).scrollTop() + 100);

          options.eventEmitter.emitEvent("scrollEnd", [visibleSection]);
        }
      });
    };
        ...
        for (var i = 0; i < sections.length; i++) {
          if (windowTop < sections[i]) {
            ...
            if (options.eventEmitter) {
              ...
              options.eventEmitter.emitEvent("scrollEnd", [nextSlideNumber]);
            }
            return false;
          }
        }
    ...
    var calculateSectionBottoms = function() {
      ...
      sectionWrapperTop = Math.round(
        sectionWrapper.position().top + parseInt(sectionWrapper.css('paddingTop'), 10) + parseInt(sectionWrapper.css('borderTopWidth'), 10) + parseInt(sectionWrapper.css('marginTop'), 10));
      ...
      sectionWrapperBottom = Math.round(
        sectionWrapperTop + sectionWrapper.height(), 10);
      tmpSections.push(sectionWrapperTop);
      $(options.sectionSelector).each(function(i, el) {
        tmpSections.push(Math.round(
          sectionWrapperTop + $(el).position().top // This will be relative to the sectionWrapper
          + $(el).outerHeight()
        ));
      });
